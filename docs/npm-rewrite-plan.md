...
做到 **`npm install -g @becomeless/cc-x` 一条命令跨平台安装**、Windows / macOS / Linux 同一套代码、
并内建 **中英文切换（i18n）**。
...
| 分发 | **npm registry**（`npm install -g @becomeless/cc-x`） | 一条命令装好，一条命令更新，零自建分发 |
...
>   最终改用**作用域包 `@becomeless/cc-x`**（作用域豁免相似名规则，且与 GitHub 品牌 `becomeless` 一致）。
>   **命令名仍是 `xx`**（`package.json` 的 `bin: { "xx": ... }`），不受包名影响。安装：`npm i -g @becomeless/cc-x`。
...
```
ccx/
  package.json                // name:"@becomeless/cc-x", bin:{xx:"./dist/index.js"}, type:module, publishConfig.access:public, files:[dist,presets.json,…]
...
- [ ] **M6 分发**：npm publish（作用域包名 `@becomeless/cc-x`，需先建 npm 组织 `becomeless` + publishConfig.access=public）；
      README 更新安装说明（`npm install -g @becomeless/cc-x`）；`npm update -g @becomeless/cc-x` 更新说明。详见 `docs/publish-guide.md`。
...
- **npm 包名**：`ccx` 已被占用；`cc-x` 撞 npm 相似名规则（归一化=`ccx`）被 E403 拒，最终定为**作用域包 `@becomeless/cc-x`**（命令名仍 `xx`）。见 §1。
...
- 2026-06-02（**包名改作用域 `@becomeless/cc-x`**）：实测 `npm publish` 时 `cc-x` 被 npm **相似名规则**以
  E403「too similar to existing package ccx」拒绝（连字符去掉后归一化=`ccx`，撞已存在的 `ccx@1.0.0`）。
  当初"`cc-x` 已确认可用"是误判——`npm view cc-x` 报 404 测不出相似名规则。改用**作用域包 `@becomeless/cc-x`**
  （作用域豁免该规则，且与 GitHub 品牌 `becomeless` 一致；命令名仍 `xx`）。已改 `package.json`（name + `publishConfig.access=public`，
  ...
  **下一步**：用户在 npmjs.com 建组织 `becomeless` → `npm publish`（首发，access 已由 publishConfig 配好）。
