...
>
> 本文件现在是 npm/TypeScript 版的实施记录与维护来源。2026-06-03 起，Bun/SEA 二进制通道跳过；
> Go 原生版作为 vNext 单独规划，见 `docs/go-rewrite-plan.md`。2026-06-04：Go 版已在 Windows 达到行为等价并进入
> GitHub Release 分发阶段；npm 版继续作为全平台 npm 安装线保留。
...
| 项 | 决策 | 理由 |
|---|---|---|
| 语言 | **TypeScript**（Node.js ≥18） | 全平台 npm 安装线；已有 Node.js 的用户安装摩擦低 |
...
| 分发 | **npm registry**（`npm install -g @cc-x/cc-x`） | npm 版继续走 npm 发布；Go 原生分发走 GitHub Releases |
| 主线 | npm/TypeScript 与 Go 原生线短期并行 | npm 保留全平台覆盖，Go 先发 Windows 原生二进制 |
...
```
ccx/
  package.json                // name:"@cc-x/cc-x", bin:{xx:"dist/index.js"}, type:module, publishConfig.access:public, files:[dist/**/*.js,!dist/release/**,presets.json,…]
