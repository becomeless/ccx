...
**cc-switch 更适合你，如果你：** 想要图形界面、需要在一个工具里统管 MCP 和多个 AI CLI、喜欢一站式。
...
- **密钥存放**：你的密钥保存在用户目录的 `~/.cc-mini/providers.json`（明文）。这是本机文件，
  受系统账户权限保护；**切勿把它提交到 Git 仓库**（已在 `.gitignore` 中排除）。
...
> 不想安装也行：直接 `pwsh -File 路径\xx.ps1` 运行同样可用。
...
欢迎 Issue / PR。设计原则只有一条、且不可妥协：**绝不写用户的配置文件**。
任何会写 `~/.claude/settings.json` 或 `~/.claude.json` 的改动都不会被接受。
