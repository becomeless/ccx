...
> 🌍 **全平台版**：一条 `npm install -g @becomeless/cc-x` 即可在 **Windows / macOS / Linux** 安装（命令仍是 `xx`，
> npm 包名是 `@becomeless/cc-x`），并内建**中英文界面切换**。Windows 上还保留了 PowerShell 版（见[安装](#安装)）。
...
装好后在任意终端敲 `xx` 即可。升级：`npm update -g @becomeless/cc-x`；卸载：`npm uninstall -g @becomeless/cc-x`。
（npm 包名是 `@becomeless/cc-x`，终端命令是 `xx`。）
...
- 卸载命令本体：
  - npm 版 → `npm uninstall -g @becomeless/cc-x`；
