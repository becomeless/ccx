...
- **Node.js ≥ 18**：npm 全平台版（`@cc-x/cc-x`）需要它。Claude Code 本身就依赖 Node，通常你已具备。
...
- 卸载命令本体：
  ...
  - 从旧 PowerShell 版迁移过来的 Windows 用户，也可删除 `$PROFILE` 里的 `# >>> xx >>>` 标记块并执行
    `Uninstall-Module ccx -AllVersions`。
- 删除数据目录 `~/.cc-mini/`。
