...
装到 `%LOCALAPPDATA%\Programs\ccx`，自动写入用户 PATH，无需手动配置。
...
装到 `~/.local/bin`，若该目录不在 PATH 中，安装器会打印一行提示（Unix 版刻意不自动改 shell 配置）。
...
只做一件事（切 API）。能不碰的就不碰，加功能前先问能不能不加。

这不是偷懒——是刻意。工具越小，出问题的面就越小。你的 MCP、插件、hooks 放在 Claude Code 自己的配置文件里就好，ccx 不替你管，也不替你弄坏。

欢迎 Issue / PR——**让它更简单的改动，比让它更强大的改动更受欢迎。** 任何会写 Claude Code 配置文件的改动都不会被接受。
...
|---|---|---|---|---|
| 官方 | 留空=登录态 | — | — | — |
| DeepSeek | `api.deepseek.com/anthropic` | `deepseek-v4-pro` | `deepseek-v4-flash` | `max`（官方推荐） |
| 智谱GLM | `open.bigmodel.cn/api/anthropic` | `GLM-4.7` | `glm-4.5-air` | — |
| 小米MiMo | `api.xiaomimimo.com/anthropic` | `mimo-v2.5-pro` | `mimo-v2.5-pro` | — |
...
2. 卸载本体：
   - Windows 原生版：重跑[安装命令](#安装)加 `-Uninstall`
   - macOS / Linux 原生版：`curl ... | sh -s -- --uninstall`；若用过设为默认，顺手删 shell 启动文件的 `# >>> xx >>>` 块
